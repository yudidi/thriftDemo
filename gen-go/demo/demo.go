...
const (
	...
	Operation_SUBSTRUCT Operation = 2
...
func OperationFromString(s string) (Operation, error) {
	...
	case "SUBSTRUCT":
		return Operation_SUBSTRUCT, nil
	case "MULTIPLY":
...
type Work struct {
	...
	Op      Operation `thrift:"op,3,required" db:"op" json:"op"`
...
func NewMyServiceProcessor(handler MyService) *MyServiceProcessor {
	...
	self0 := &MyServiceProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	return self0
}
...
type Calculator interface {
	...
	//  - LogId
	//  - Work
	Calculate(logId int32, work *Work) (r int32, err error)
	Zip() (err error)
}
...
func NewCalculatorClientFactory(t thrift.TTransport, f thrift.TProtocolFactory) *CalculatorClient {
	return &CalculatorClient{MyServiceClient: NewMyServiceClientFactory(t, f)}
}

func NewCalculatorClientProtocol(t thrift.TTransport, iprot thrift.TProtocol, oprot thrift.TProtocol) *CalculatorClient {
	return &CalculatorClient{MyServiceClient: NewMyServiceClientProtocol(t, iprot, oprot)}
}
...
//  - LogId
//  - Work
func (p *CalculatorClient) Calculate(logId int32, work *Work) (r int32, err error) {
	if err = p.sendCalculate(logId, work); err != nil {
		return
	}
	return p.recvCalculate()
}

func (p *CalculatorClient) sendCalculate(logId int32, work *Work) (err error) {
	oprot := p.OutputProtocol
	if oprot == nil {
		oprot = p.ProtocolFactory.GetProtocol(p.Transport)
		p.OutputProtocol = oprot
...
func NewCalculatorProcessor(handler Calculator) *CalculatorProcessor {
	self8 := &CalculatorProcessor{NewMyServiceProcessor(handler)}
...
func (p *calculatorProcessorCalculate) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	var retval int32
	var err2 error
	if retval, err2 = p.handler.Calculate(args.LogId, args.Work); err2 != nil {
		switch v := err2.(type) {
...
var CalculatorCalculateArgs_Work_DEFAULT *Work

func (p *CalculatorCalculateArgs) GetWork() *Work {
	if !p.IsSetWork() {
		return CalculatorCalculateArgs_Work_DEFAULT
	}
	return p.Work
}
func (p *CalculatorCalculateArgs) IsSetWork() bool {
	return p.Work != nil
}
